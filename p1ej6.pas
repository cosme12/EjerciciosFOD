program p1ej6; //dentro de todo bien, tiene algunos errores de ejecucion
type
regCelular=record
...
procedure anadirCelulares (var archibin:bin);
  var
    celular:regCelular;
  begin
    reset(archibin);
    seek(archibin,filesize(archibin)-1);
    writeln('Para dejar de ingresar, ingrese el codigo 000.');
    leerCelular(celular);
    while (celular.codigo<>000) do begin
      write(archibin,celular);
      leerCelular(celular);
...
procedure modificarStock (var archibin:bin);
  var
    celular:regCelular; nombre:string; stock:integer;
  begin
    reset(archibin);
    write('Ingrese el nombre del celular a modificar: ');readln(nombre);
    read(archibin,celular);
    while (not eof(archibin)) and (nombre<>celular.nombre) do begin
      read(archibin,celular);
    end;
    if (nombre=celular.nombre) then begin
      seek(archibin,filepos(archibin)-1);
      write('Ingrese el nuevo stock disponible: ');readln(stock);
      celular.stockDis:=stock;
      write(archibin,celular)
    end
    else
...
procedure exportarSinStock (var archibin:bin);
  var
    celular:regCelular; txt:text;
  begin
  ...
  begin
    ...
    if (opcion<>0) then begin
      case opcion of
        ...
        5: anadirCelulares(archibin);
        6: modificarStock(archibin);
        7: exportarSinStock(archibin);
        else
